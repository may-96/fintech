                                                            ...
                                                            <div class="d-flex w-100">
                                                                <div class="w-100">
                                                                    {{-- <p class="ficon m-0 w-100 lh1_3 text-start pointer" x-on:click="$store.data.toggleTransaction('timeline',{{ $transaction['id'] }})"> --}}
                                                                        ...
                                                                        <pre style="font-family: 'Manrope', sans-serif;white-space: pre-wrap;" x-on:click="$store.data.toggleTransaction('timeline',{{ $transaction['id'] }})" class="p-0 ficon m-0 w-100 lh1_3 text-start pointer mb-1 fw-bold text-dark d-block fs-14">@if(App\Helpers\Functions::not_empty($transaction['remit_info_unstructured'])) {!! nl2br($transaction['remit_info_unstructured']) !!} @elseif(App\Helpers\Functions::not_empty($transaction['remittance_information_unstructured_array'])) {!! (implode("\n",json_decode($transaction['remittance_information_unstructured_array']))) !!} @elseif(App\Helpers\Functions::not_empty($transaction['remittance_information_structured'])) {!! nl2br($transaction['remittance_information_structured']) !!} @elseif(App\Helpers\Functions::not_empty($transaction['remittance_information_structured_array'])) {!! (implode("\n",json_decode($transaction['remittance_information_structured_array']))) !!} @endif<span><i id="transaction_timeline_icon_{{ $transaction['id'] }}" x-bind:class="$store.data.is_expanded({{ $transaction['id'] }}) ? 'uil-arrow-up' : 'uil-arrow-down'" class="uil v-middle fs-20 text-muted"></i></span></pre>
                                                                    {{-- </p> --}}
                                                                    ...
                                                                    <div x-bind:class="$store.data.is_expanded({{ $transaction['id'] }}) ? 'transaction_expanded' : 'transaction_collapsed'" class="text-start mb-1 transaction_timeline_{{ $transaction['id'] }}">
                                                                        ...
                                                                        <div class="my-2 text-dark fs-15 fw-bold">Data From Premium EndPoint</div>
                                                                        ...
                                                                        @if(App\Helpers\Functions::not_empty($transaction['enriched_website']))<div class="w-100 text-dark fs-14 text-start row m-0 mb-1"><div class="col-12 col-lg-5 lh1_3 p-0"><small><strong>Merchant Website: </strong> </small></div><div class="col-12 col-lg-7 lh1_3 p-0"> <a href="{{ $transaction['enriched_website'] }}" target="_blank">{{$transaction['enriched_website'] }}</a></div></div>@endif

                                                                        @if(App\Helpers\Functions::not_empty($transaction['enriched_name']) || App\Helpers\Functions::not_empty($transaction['enriched_logo']))<div class="w-100 text-dark fs-14 text-start row m-0 mb-1 mb-lg-0"><div class="col-md-12 col-lg-5 lh1_3 p-0"><small><strong>Merchant Logo: </strong></small></div><div class="col-md-12 col-lg-7 lh1_3 p-0">@if(App\Helpers\Functions::not_empty($transaction['enriched_logo'])) <img style="max-width: 26px; max-height: 26px;" src="{{ $transaction['enriched_logo'] }}" alt="Logo"> @endif @if(App\Helpers\Functions::not_empty($transaction['enriched_name'])) <span>{{ $transaction['enriched_name'] }}</span> @endif </div> </div>@endif
                            ...
                            <div class="list-group w-100">
                                <div class="d-flex w-100 ">
                                    <div class="ficon m-0 w-100 text-start t2_para" >
                                        {{-- <small class="text-muted fw-normal fs-11">12:02:46 PM</small> --}}
                                        <pre style="font-family: 'Manrope', sans-serif;white-space: pre-wrap;" class="p-0 d-block fw-bold text-dark fs-14 pointer" x-on:click="$store.data.toggleTransaction('timeline_2',{{ $transaction['id'] }})">@if(App\Helpers\Functions::not_empty($transaction['remit_info_unstructured'])) {!! nl2br($transaction['remit_info_unstructured']) !!} @elseif(App\Helpers\Functions::not_empty($transaction['remittance_information_unstructured_array'])) {!! (implode("\n",json_decode($transaction['remittance_information_unstructured_array']))) !!} @elseif(App\Helpers\Functions::not_empty($transaction['remittance_information_structured'])) {!! nl2br($transaction['remittance_information_structured']) !!} @elseif(App\Helpers\Functions::not_empty($transaction['remittance_information_structured_array'])) {!! (implode("\n",json_decode($transaction['remittance_information_structured_array']))) !!} @endif <span><i id="transaction_timeline_2_icon_{{ $transaction['id'] }}" x-bind:class="$store.data.is_expanded({{ $transaction['id'] }}) ? 'uil-arrow-up' : 'uil-arrow-down'" class="uil v-middle fs-18 text-muted"></i></span></pre>
